...
namespace DrawAirplan
{
    public class AircraftComparer : IComparer<Vehicle>
    {
        ...
        private int ComparerAirbus(Airbus x, Airbus y)
        {
            ...
            if (x.AirplanChassis != y.AirplanChassis)
            {
                return x.AirplanChassis.CompareTo(y.AirplanChassis);
            }
            if (x.LowerWindows != y.LowerWindows)
            {
                return x.LowerWindows.CompareTo(y.LowerWindows);
            }            
            return 0;
        }
